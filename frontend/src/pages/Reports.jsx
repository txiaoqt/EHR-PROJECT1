...
const Reports = () => {
  ...
  const exportPdf = async (mode = 'selected') => {
    ...
    try {
      ...
      // --- COVER PAGE (page 1) with logo + header text ---
      doc.setFontSize(22);
      doc.setFont('helvetica', 'bold');
      if (logoDataUrl) {
        try { doc.addImage(logoDataUrl, 'PNG', (pageW / 2) - 40, 80, 80, 60); } catch (e) { /* ignore */ }
      }
      doc.text('Clinic Reports & Analytics', pageW / 2, 170, { align: 'center' });
      doc.setFontSize(12);
      doc.setFont('helvetica', 'normal');
      doc.text(`Type: ${type}`, pageW / 2, 190, { align: 'center' });
      doc.text(`Date Range: ${rangeText}`, pageW / 2, 208, { align: 'center' });
      doc.text(`Generated: ${formatDate(new Date(), 'YYYY-MM-DD')}`, pageW / 2, 226, { align: 'center' });
